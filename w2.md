...
# 타입 변환, 캐스팅 그리고 타입 프로모션
- 사이즈가 큰 타입에 작은 타입으로 저장하는 경우 자동 타입 변환이 가능하다. <br/>
- 반대로 사이즈가 작은 타입에 큰 타입을 저장하는 경우에는 자동 타입 변환이 불가능하고 <br/>
강제 타입 변환을 사용해서 강제로 값을 잘라서 저장할 수 있다. (사이즈가 작은 타입보다 사이즈가 넘어서는 값은 버려지게 된다.) <br/>
때문에 강제 타입 변환을 사용할 때에는 값이 손실되기 때문에 주의해서 사용해야 한다.  <br/>
- 실수 타입(float, double)은 정수 타입으로 강제 변환하면 소수점이 버려진다. (자동 타입 변환되지 않는다.)  <br/>
<br/><br/><br/><br/>
