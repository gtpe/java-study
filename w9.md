...
# 자바에서 예외 처리 방법 (try, catch, throw, throws, finally)
자바에는 약간 다른 두가지 예외처리 메커니즘이 있다.<br/>
하나는 try/catch/finally 문이다.<br/>
try절은 예외 처리를 하려는 코드를 작성하는 블록이고<br/>
try 블록에는 0개 이상의 catch절이 따라온다.<br/>
catch절은 처리할 예외를 나타낸다.<br/>
마지막으로 finally는 try 블록에서 발생하는 상황에 관계없이 실행되는 코드를 작성하는 블록이며,<br/>
finally가 필요하지 않으면 반드시 명시할 필요는 없다.<br/>
