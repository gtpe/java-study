...
# RuntimeException과 RE가 아닌 것의 차이는?
unchecked은 명시적으로 처리하지 않아도 된다. 

예외처리를 강제하고 싶을 경우에는 checked, 강제하고 싶지 않을 경우에는 unchecked 예외를 던지도록 만들면 된다.
보통의 경우 RuntimeException이 발생하지 않도록 미리 예측하고 코드를 작성하여 예외가 발생하지 않도록 처리하는 것이 가능하다.
